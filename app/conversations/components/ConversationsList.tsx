...
const ConversationsList = ({ initialItems, users }: ConversationsListProps) => {
  ...
  useEffect(() => {
    ...
    const deleteHandler = (conversationToDelete: FullConversationType) => {
      setItems(current => {
        return [...current.filter(currentConversation => currentConversation.id !== conversationToDelete.id)]
      })

      if (items.some(conversation => conversation.id === conversationToDelete.id)) {
